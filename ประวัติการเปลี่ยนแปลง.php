...
<body>
  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active"><a href="ประวัติการเปลี่ยนแปลง.php">ประวัติการเปลี่ยนแปลง</a></li>
</ol>
  </nav>
</div><!-- End Page Title -->
    <style>
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<h3>ข้อมูลการเพิ่มสิทธิ์</h3>";
                echo "<table>
                        <tr>
                ...
                while ($row = $result->fetch_assoc()) {
                    if ($row["ActionType"] === "เพิ่มสิทธิ์") { // เฉพาะการเพิ่ม
                        echo "<tr>";
                        echo "<td>" . $order++ . "</td>"; // แสดงลำดับและเพิ่มขึ้น
                        echo "<td><a href='employee_tasks.php?employee_id=" . urlencode($row["EmployeeID"]) . "'>" . htmlspecialchars($row["EmployeeID"]) . "</a></td>";
                        echo "<td>" . htmlspecialchars($row["FullName"]) . "</td>";
                        echo "<td>" . htmlspecialchars($row["TaskName"]) . "</td>";
                        echo "<td>" . ($row["Start"] ? thai_date($row["Start"]) : '-') . "</td>"; // ใช้วันที่ภาษาไทย
                        echo "<td>" . ($row["End"] ? thai_date($row["End"]) : '-') . "</td>"; // ใช้วันที่ภาษาไทย
                ...
                while ($row = $result->fetch_assoc()) {
                    if ($row["ActionType"] === "ลบสิทธิ์") { // เฉพาะการลบ
                        ...
                        echo "<td><a href='employee_tasks.php?employee_id=" . urlencode($row["EmployeeID"]) . "'>" . htmlspecialchars($row["EmployeeID"]) . "</a></td>";
                        echo "<td>" . htmlspecialchars($row["FullName"]) . "</td>";
                        echo "<td>" . htmlspecialchars($row["TaskName"] ?? '-') . "</td>"; // ใช้ null coalescing operator เพื่อตรวจสอบ TaskName
                        echo "<td>" . htmlspecialchars($row["Status"]) . "</td>";
                        echo "<td>" . htmlspecialchars($row["ActionType"]) . "</td>";
                        echo "<td>" . ($row["DeletedAt"] ? thai_date($row["DeletedAt"]) : '-') . "</td>"; // ใช้วันที่ภาษาไทย
                        echo "</tr>";
                    }
                }
                echo "</table>";
            } else {
                echo "<p>ไม่มีข้อมูลการเพิ่มหรือลบสิทธิ์สำหรับพนักงานในแผนกนี้</p>";
            }
