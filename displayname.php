...
if (count($employees) > 0) {
    foreach ($employees as $index => $employee) {
        ...
        // จัดตำแหน่งให้ชิดขวา
        echo "<td style='text-align: right;'>" . ($index + 1) . "</td>";
        
        // เพิ่มลิงก์ที่คลิกได้ใน EmployeeID ไปยัง 55021.php
        echo "<td style='text-align: center;'><a href='55021.php?employeeID=" . urlencode($employee['EmployeeID']) . "'>" 
        . htmlspecialchars($employee['EmployeeID']) . "</a></td>";
        // รวมชื่อและนามสกุลเข้าด้วยกัน
        echo "<td>" . htmlspecialchars($employee['FirstName'] . " " . $employee['LastName']) . "</td>";
        
        echo "<td>" . htmlspecialchars($employee['Position']) . "</td>";
        echo "</tr>";
    }
} else {
    echo "<tr><td colspan='4'>ไม่มีข้อมูลพนักงานในแผนกนี้</td></tr>"; // เปลี่ยน colspan เป็น 4 เนื่องจากมี 4 คอลัมน์
}
?>
